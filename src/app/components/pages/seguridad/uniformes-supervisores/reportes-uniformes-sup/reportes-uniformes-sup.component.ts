...
@Component({
  selector: 'app-reportes-uniformes-sup',
  templateUrl: './reportes-uniformes-sup.component.html',
  styleUrls: ['./reportes-uniformes-sup.component.css']
})
export class ReportesUniformesSupComponent implements OnInit {
  ...
  regresar(){
    this._router.navigate(['pages/seguridad/UniformesSupervisores']);
  }
  ...
  generarReporte(){
    ...
    this.aprobacionesService.reporteFactorUniformeSup(this.fechaRango1, this.fechaRango2).subscribe({
      next: (response: any) => {
        ...
        if(this.dataAgentes.length > 0){
          ...
          let reportData = {
            title: 'FACTOR UNIFORME(SUPERVISORES)',
